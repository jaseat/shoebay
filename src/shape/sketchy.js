...
/**
 * Takes in an array of paths and outputs it in Raphael SketchPad-style JSON/SVG data.
 * @typedef {Object} point
 * @property {number} x X coordinate
 * @property {number} y Y coordinate
 * @typedef {point[]} points
 * @param {points[]} paths Array of points arrays.
 */
...
/**
 * Takes stringified JSON data from Raphael SketchPad and returns similarity.
 * @param {string} shape1 stringified JSON data from Raphael SketchPad
 * @param {string} shape2 stringified JSON data from Raphael SketchPad
 * @returns {number} Similarity between shape 1 and 2 between 0 and 1.
 */
Sketchy.shapeContextMatch = function(shape1, shape2) {
  ...
  // not the shapes.
  points1 = Sketchy.scatterPoints(
    Sketchy.convertSVGtoPointArrays(shape1),
    pointsPerShape
  );
  points2 = Sketchy.scatterPoints(
    Sketchy.convertSVGtoPointArrays(shape2),
    pointsPerShape
  );
  ...
  for (i = 0; i < pointsPerShape - 1; i++) {
    for (j = i + 1; j < pointsPerShape; j++) {
      distanceMatrix1[i][j] = distanceMatrix1[j][i] = Sketchy.euclideanDistance(
        points1[i].x,
        points1[i].y,
        points1[j].x,
        points1[j].y
      );
      distanceMatrix2[i][j] = distanceMatrix2[j][i] = Sketchy.euclideanDistance(
        points2[i].x,
        points2[i].y,
        points2[j].x,
        points2[j].y
  ...
  for (i = 0; i < pointsPerShape - 1; i++) {
    for (j = i + 1; j < pointsPerShape; j++) {
      ...
      angleMatrix1[i][j] =
        (Math.atan2(points1[j].y - points1[i].y, points1[j].x - points1[i].x) +
          2 * Math.PI) %
        (2 * Math.PI);
      angleMatrix2[i][j] =
        (Math.atan2(points2[j].y - points2[i].y, points2[j].x - points2[i].x) +
          2 * Math.PI) %
        (2 * Math.PI);
  ...
  for (i = 0; i < pointsPerShape; i++) {
    for (j = 0; j < pointsPerShape; j++) {
      angleBins1[i][j] =
        1 + Math.floor(angleMatrix1[i][j] / ((2 * Math.PI) / angleBinCount));
      angleBins2[i][j] =
        1 + Math.floor(angleMatrix2[i][j] / ((2 * Math.PI) / angleBinCount));
    }
  }
    ...
    for (j = 0; j < pointsPerShape; j++) {
      ...
      for (logr = 1; logr <= distanceBinCount; logr++) {
        for (theta = 1; theta <= angleBinCount; theta++) {
          ...
          hik = Sketchy.shapeContextHistogram(
            i,
            logr,
            theta,
            distanceBins1,
          ...
          hjk = Sketchy.shapeContextHistogram(
            j,
            logr,
            theta,
            distanceBins2,
...
Sketchy.shapeContextHistogram = function(
  pointIndex,
  distanceBinNumber,
  angleBinNumber,
  distanceBins,
...
Sketchy.scatterPoints = function(paths, numberOfPoints) {
  ...
  lengthNotCovered = 0;
  for (i = 0; i < paths.length; i++) {
    lengthNotCovered += Sketchy.computeLength(
      paths[i],
      Sketchy.euclideanDistance
    );
  }
  ...
  for (i = 0; i < paths.length; i++) {
    ...
    numberOfPointsForPath = Math.round(
      (pathLength / lengthNotCovered) * pointsNotAssigned
    );
    if (i === paths.length - 1) {
...
Sketchy.distributePointsAcrossPath = function(path, numberOfPoints) {
  ...
  for (i = 1; i < numberOfPoints - 1; i++) {
    distanceCovered = 0;
    do {
      distanceToNextPoint = Sketchy.euclideanDistance(
        currX,
        currY,
        path[nextPathIndex].x,
        path[nextPathIndex].y
      );
      ...
      if (distanceToNextPoint <= delta - distanceCovered) {
        ...
        // Move partially
        angleToNextPoint = Math.atan2(
          path[nextPathIndex].y - currY,
...
Sketchy.hausdorff = function(points1, points2, vector2D) {
  ...
  for (var i = 0; i < points1.length; i++) {
    ...
    for (var j = 0; j < points2.length; j++) {
      dis = Sketchy.euclideanDistance(
        points1[i].x,
        points1[i].y,
        points2[j].x + vector2D.x,
        points2[j].y + vector2D.y
      );
      if (dis < h_min) {
        h_min = dis;
      } else if (dis == 0) {
